...
update_sigma2 = function(Y,a,b,S2,res,var.type,S2.type,energy,energy.S2){
  ...
  if(var.type=='by_row'){
    if(S2.type=='zero'){
      sigma2 = c(rowMeans(get_R2_e(Y,energy,res$Fe,a,b,res$EL,res$EF,res$EL2,res$EF2)$R2_all))
    }else{
      R2 = get_R2_e(Y,energy,res$Fe,a,b,res$EL,res$EF,res$EL2,res$EF2)$R2_all
      S2_e = cbind(S2,energy.S2)
      sigma2 = rep(0,n)
      for(r in 1:n){
        sigma2[r] = optimize_sigma2(R2[r,],S2_e[r,])
      }
    }
    sigma2 = tcrossprod(sigma2,rep(1,p+1))
